...
from apps.analyze.analyze_tag_ranking_embedding import TagPriorityGenerator  # 태그 우선순위 생성기
import json
...
class TagPriorityManager:
    ...
    def __init__(self, tagged_data_path, priority_data_path, tags=None):
        ...
        print('TagPriorityManager 인스턴스 생성')
...
def ranking_run(senario, case_id) :
    ...
    priority_data_path = os.path.join(case_folder, 'tag_priority_data.json')
    tagged_data_path = os.path.join(case_folder, 'tagged_data_add_upload.json')

    if not os.path.exists(priority_data_path):
        # TagPriorityGenerator 인스턴스 생성
        tag_generator = TagPriorityGenerator(api_key)

        # 우선순위 데이터를 생성하고 저장
        tag_generator.save_priority_data(scenario_text, tags, output_path=priority_data_path)
