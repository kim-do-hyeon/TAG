...
def all_table_parsing(data, user) :
    ...
    # 테이블 목록 가져오기
    progress_bar.set_now_log('SELECT name FROM sqlite_master WHERE type="table";')
    tables_query = 'SELECT name FROM sqlite_master WHERE type="table";'
    tables = pd.read_sql_query(tables_query, conn)
    # 모든 테이블 데이터를 저장할 사전
    all_data = {}
    # 특정 테이블에 대해 작업
    progress_bar.start_progress(len(tables['name']) + 1)
    for table_name in tables['name']:
        progress_bar.done_1_task()
        try:
            query = f'SELECT * FROM "{table_name}"'
            progress_bar.set_now_log(query)
    ...
    progress_bar.set_now_log('save to ' + output_path + '...')
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(all_data, f, indent=4)
    progress_bar.done_1_task()
    print("모든 테이블 데이터가 'all_tables_data_email_phishing.json' 파일에 저장되었습니다.")
    progress_bar.set_now_log("모든 테이블 데이터가 'all_tables_data_email_phishing.json' 파일에 저장되었습니다.")
    conn.close()
...
def all_tag_process(data, user, json_data, output_path) :
    user = user
    case_id = data['case_id']
